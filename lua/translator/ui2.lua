...
function M.create_floating_window(source_text)
	...
	-- 窗口总高度 = 原文高度 + 译文高度(初始为2) + 分隔线(1) + 边框(2)
	local total_height = height + 2 + 1 + 2
...
-- 更新翻译内容
function M.update_translation(bufnr, translation, is_streaming)
	...
	-- 清除旧的翻译内容
	vim.api.nvim_buf_set_lines(bufnr, separator_line + 1, -1, false, {})

	-- 添加新的翻译内容
	local translation_lines = vim.split(translation, "\n")
	vim.api.nvim_buf_set_lines(bufnr, separator_line + 1, -1, false, translation_lines)
	...
	if is_streaming then
		...
		if winnr ~= -1 then
			...
			-- 窗口总高度 = 原文高度 + 译文高度 + 分隔线(1) + 边框(2)
			local total_height = source_height + translation_height + 1 + 2

			-- 调整窗口高度
			vim.api.nvim_win_set_height(winnr, total_height)
...
function M.calculate_height(text, width)
	...
	for _, line in ipairs(lines) do
		...
		local line_height = math.max(1, math.ceil(line_length / width))
